...
LANGUAGES = ["anarchy_inference_optimized", "python", "javascript", "rust"]
...
def analyze_code_samples():
    ...
    for task in TASKS:
        ...
        for lang in LANGUAGES:
            # Handle the optimized version of Anarchy Inference
            if lang == "anarchy_inference_optimized":
                file_extension = ".ai"
...
def calculate_reduction_percentages(results):
    ...
    anarchy_tokens = results["token_counts"]["anarchy_inference_optimized"]
    
    reductions = {}
    for lang in [l for l in LANGUAGES if l != "anarchy_inference_optimized"]:
...
def generate_visualizations(results, reductions):
    ...
    # Get reduction data
    langs = [lang for lang in LANGUAGES if lang != "anarchy_inference_optimized"]
...
"""
    ...
    for i, task in enumerate(results["tasks"]):
        ai_tokens = results["token_counts"]["anarchy_inference_optimized"][i]
    ...
    # Add average row
    ai_avg = sum(results["token_counts"]["anarchy_inference_optimized"]) / len(results["tasks"])
...
"""
    ...
    # Add conclusion
    min_reduction = min([reductions[f"vs_{lang}"]["average_reduction"] for lang in [l for l in LANGUAGES if l != "anarchy_inference_optimized"]])
...
Anarchy Inference demonstrates a consistent {min_reduction:.1f}%+ reduction in token usage compared to mainstream programming languages. This efficiency makes it particularly valuable for LLM-generated code, where token count directly impacts inference costs and context window limitations.
...
"""
    ...
    with open(os.path.join(OUTPUT_DIR, 'benchmark_report_optimized.md'), 'w', encoding='utf-8') as f:
        f.write(report)
    ...
    print(f"Report generated: {os.path.join(OUTPUT_DIR, 'benchmark_report_optimized.md')}")
    return report

def main():
    print("Running Anarchy Inference Token Efficiency Benchmark with Optimized Code...")
    ...
    with open(os.path.join(OUTPUT_DIR, 'benchmark_data_optimized.json'), 'w', encoding='utf-8') as f:
        json.dump({
            "results": results,
